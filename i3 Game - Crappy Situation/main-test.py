...
#    def test(self,player):
 #       if player.x < self.x1 or player.x > self.x2:return None
  #      if player.y <= self.y and player.y + player.vel >= self.y:return self
   #     return None
...
#    def draw(self):
 #       display = pygame.display.get_surface()
  #      for p in self.container:
   #         pygame.draw.line(display, (255,255,255), (p.x1,p.y),(p.x2,p.y), 3)
#    def do(self,player):
...
while run:
    ...
    if man.y > 210 and man.y < 235:
        if man.x < 240 and man.x > 140:
            man.isJump = False
